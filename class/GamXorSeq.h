...
class GamXorSeq
{
  private:
    ...
    void setkeyword();
  public:
    GamXorSeq(string operationfile_val, int A_val, int C_val);
    GamXorSeq(string operationfile_val);
    ~GamXorSeq();

    void codefile();
    void decodefile();
    friend wostream &operator<<(wostream &stream, GamXorSeq o);
};

void GamXorSeq::setkeyword()
{
  wcout << L" Генерация ключа" << endl;
  srand((unsigned)time(NULL));
  int T0 = abs(rand());
  int m = alphabet.length();
  int n = abs(rand()) % m; // future keyword length
  int Ti = abs((A * T0 + C) % m); // if remove abs then strange errors occure
  int Tplus1 = 0;
  for(int i = 0; i < n; i++)
  {
    progress_bar(i + 1, n);
    wchar_t ch = alphabet[Ti];
    if(ch == L' ' || ch == L'\n' || ch == L'\t')
    {
      i--;
      Tplus1 = (A * Ti + C) % m;
...
GamXorSeq::GamXorSeq(string operationfile_val, int A_val, int C_val)
{
...
GamXorSeq::GamXorSeq(string operationfile_val)
{
  operationfile = operationfile_val;
  A = 0;
  C = 0;
...
wostream &operator<<(wostream &stream, GamXorSeq o)
{
  stream << endl;
